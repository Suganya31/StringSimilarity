...
//System.out.println(filteredTokens.size());
		...
		{ j=i+1;
			while(j < filteredTokens.size()) {
				//System.out.println("the values of i and j are"+i+" j is"+j);

			 overlap = findOverlap(filteredTokens.get(i),filteredTokens.get(j));
			 Pair<ArrayList<String>, ArrayList<String>> pair = new Pair<>(filteredTokens.get(i),filteredTokens.get(j));
